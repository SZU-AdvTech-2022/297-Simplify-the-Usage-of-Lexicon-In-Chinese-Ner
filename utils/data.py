...
class Data:
            ...
            if len(line) > 3:
                ...
                for idx in range(w_length):
                    ...
                    for entity in matched_entity:
                        ...
                        # 高 在instance2index(dic)中的index为3 但在index2instance(数组)中下标为2
                ...
                # 开始计数
                if count:
                    # entitys保留当前句子匹配到的所有词汇，包含重复匹配的词
    ...
    def build_gaz_pretrain_emb(self, emb_path):
        ...
        # 根据预训练的gaz词典找出训练集中的字词所用的embedding
    ...
    def generate_instance_with_gaz(self, input_file, name):
        ...
        if name == "train":
            ...
            self.raw_texts, self.raw_Ids = read_instance_with_gaz1(self.HP_num_layer, input_file, self.gaz, self.word_alphabet,self.biword_alphabet, self.biword_count, self.char_alphabet, self.gaz_alphabet, self.gaz_count, self.gaz_split,  self.label_alphabet, self.number_normalized, self.MAX_SENTENCE_LENGTH)
        else:
    ...
    def build_alphabet1(self, input_file):
        in_lines = open(input_file,'r',encoding="utf-8").readlines()
        seqlen = 0
        for idx in range(len(in_lines)):
            line = in_lines[idx]
    ...
    def build_gaz_alphabet1(self, input_file, count=False):
        in_lines = open(input_file,'r',encoding="utf-8").readlines()
        word_list = []
        for line in in_lines:
            if len(line) == 2:
