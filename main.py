...
def data_initialization(data, gaz_file, train_file, dev_file, test_file):
    # alphabet包括id2instance和instance2index
    # 以下操作对每个char、word、biword都统计了相应的count
    # 产生训练集的 char、word、biword字母表（instance2id,id2instance）没提取embedding
    data.build_alphabet(train_file)
    data.build_alphabet(dev_file)
    data.build_alphabet(test_file)
    # 产生 gaz词典中的所有字词的表 entity2id、entity2type，生成词典树
    data.build_gaz_file(gaz_file)
    # 借助gazetteer词典对训练集中的每个字找出匹配词，生成instance2index和id2instances表，这两个表示相差一的，没有生成embedding
    # 对每个匹配词都计算了相应的count
...
def train(data, save_model_dir, seg=True):
    ...
    for idx in range(data.HP_iteration):
        ...
        for batch_id in range(total_batch):
            ...
            # 已经是补充了padding之后的各种向量
            # layer_gaz是BMES四组向量并补充了0之后的
            # gaz_count记录了BMES四组向量里面的词的出现频率
            # gaz_chars应该是记录BMES四组向量里面的词中的字的信息
            # gaz_mask记录了BMES四组向量的mask信息
...
if __name__ == '__main__':
    ...
    parser.add_argument('--modelname', default="model_GAZLSTM")
    ...
    if status == 'train':
        if os.path.exists(save_data_name):
            ...
            data.model_type = args.model_type
            data.use_bert = args.use_bert
            # 1.利用三个文件生成总字母表(包括四种类型)
            # 2.从gaz文件生成匹配树
            # 3.对三个文件从匹配树匹配出来的词来构造gaz_alphabet并记录count
            data_initialization(data, gaz_file, train_file, dev_file, test_file)
            # 以每个句子为单位
            # 生成BMES四个数组以及对应的词的次数的数组、以及补充padding和生成mask数组、根据训练集句子生成对应的word、char、biword数组
            data.generate_instance_with_gaz(train_file,'train')
            data.generate_instance_with_gaz(dev_file,'dev')
            data.generate_instance_with_gaz(test_file,'test')
            #根据训练集生成的chrr、word、biword、gaz_alphabet、从预定义的emb来获得相应的 pre_train_embedding
        ...
        data.HP_use_count = args.use_count
        data.generate_instance_with_gaz(test_file,'test')
        pre=load_model_decode(save_model_dir, data, 'test', gpu, seg)
        print_results(pre,'')
    else:
